...
def generate_flirt_bbr_svg(subject: str, task: str, run: str):
    ...
    out_file_duplicate = "/cds2/abcd/cold/derivatives/fmriprep_missing_svgs/%s/ses-baselineYear1Arm1/func/%s_ses-baselineYear1Arm1_task-%s_run-%s_desc-flirtbbr_bold.svg" % (subject, subject, task, run)
...
funcs = glob.iglob("/cds2/abcd/cold/derivatives/fmriprep/*/ses-baselineYear1Arm1/func/*_ses-baselineYear1Arm1_task-*_run-*_space-MNI152NLin6Asym_desc-preproc_bold.nii.gz")
funcs = list(filter(lambda func: not Path(func.replace('space-MNI152NLin6Asym_desc-preproc_bold.nii.gz','desc-flirtbbr_bold.svg')).exists(), funcs))

for func in funcs:
    subject = str(Path(func).parent.parent.parent.stem)
    task = str(Path(func).stem).split('task-')[-1].split('_')[0]
    run = str(Path(func).stem).split('run-')[-1].split('_')[0]
    generate_flirt_bbr_svg(subject, task, run)
