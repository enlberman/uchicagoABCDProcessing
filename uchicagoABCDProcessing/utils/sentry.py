...
def start_ping(run_uuid, npart):
    ...
    sentry_sdk.add_breadcrumb(message='uchicagoABCDProcessing started', level='info')
    sentry_sdk.capture_message('uchicagoABCDProcessing started', level='info')
...
def sentry_setup(opts, exec_env):
    ...
    if not __version__:
        environment = "dev"
        release = "dev"
    elif int(os.getenv('uchicagoABCDProcessing_DEV', '0')) or ('+' in __version__):
        environment = "dev"
